(function(){
    ...
    // Improvement: capture the last user-sent message so the popup can suggest it
    // even if the message hasn't yet been appended to window.messages.
    ...
    var _lastUserSent = null; // NEW: remember last user-sent message (id, text, timestamp)
    ...
    function findRelevantUserMessage(partnerMsg){
        ...
        if(msgs && msgs.length){
            // attempt to find a user-sent message immediately preceding the partner message
            var idx = -1;
            ...
            for(var j = start; j >= 0; j--){
                var m = msgs[j];
                if(!m) continue;
                // consider common user markers: sender==='user' OR sender==='me' OR role==='user'
                var isUser = (m.sender === 'user' || m.sender === 'me' || m.role === 'user');
                if(isUser && m.type !== 'system' && (m.text || m.image)){
                    return m;
                }
            }
            // fallback: last user message anywhere in the array
        ...
        // NEW: final fallback to the last captured user-sent message (may not yet be in window.messages)
        if(_lastUserSent && _lastUserSent.text) {
            return _lastUserSent;
        }
        return null;
    }
    ...
    function maybePromptOnPartnerMessage(partnerMsg){
        try{
            ...
            if(!_shouldProcessMessage(partnerMsg)){
                log('skip old/processed partnerMsg', partnerMsg && (partnerMsg.id || partnerMsg.timestamp));
        ...
        try{
            if(typeof window.addMessage === 'function' && !window.__pcp_addMessageHooked){
                ...
                window.addMessage = function(msg){
                    ...
                    try{
                        // capture last user-sent message here
    ...
    // Wrap sendMessage (if exists) to capture last sent text immediately
        ...
        try{
            if(typeof window.sendMessage === 'function' && !window.__pcp_sendMessageWrapped){
                ...
                window.sendMessage = function(){
                    try{
                        // heuristics: if first arg is a string or object with text, capture it
                        var a0 = arguments[0];
                        if(typeof a0 === 'string'){
                            _lastUserSent = { id: Date.now(), text: a0, timestamp: new Date().toISOString(), sender: 'user', type: 'normal' };
                            log('captured lastUserSent from sendMessage string', _lastUserSent);
                            ...
                            log('captured lastUserSent from sendMessage object', _lastUserSent);
                        } else {
                            // If no arg, try to read from message input element if present
                            try{
                                var inp = document.getElementById('message-input') || document.querySelector('textarea, input[data-send-message]');
                                if(inp && inp.value) {
                                    _lastUserSent = { id: Date.now(), text: inp.value, timestamp: new Date().toISOString(), sender: 'user', type: 'normal' };
                                    log('captured lastUserSent from DOM input', _lastUserSent);
                                }
                            }catch(e){}
                        }
                    }catch(e){}
                ...
                try{
                    ...
                    if(arr.length > lastLen){
                        for(var i = lastLen; i < arr.length; i++){
                            ...
                            // also capture user messages pushed to messages
                            ...
                            if(!sender){
                                if(node.classList && (node.classList.contains('message-wrapper')||node.classList.contains('message'))){
                                    ...
                                    if(isPartner){
                                        ...
                                        // if it's a 'me' node, capture lastUserSent from DOM
                                ...
                                if(sender !== 'user' && sender !== 'me'){
                                    ...
                                    // capture user from DOM node with explicit sender
    ...
    function installHooks(){
        ...
        // mark existing messages processed (prevent prompts for historic messages)
        try{ if(Array.isArray(window.messages)){ window.messages.forEach(function(m){ if(m && m.id !== undefined) _processedIds.add(m.id); }); } }catch(e){}
        installAddMessageHook();
        tryWrapSendMessage();
        startMessagesPoll();
        startDOMObserver();
    ...
    window.PartnerCardPrompt = window.PartnerCardPrompt || {
        ...
        _debug: { installHooks: installHooks, startMessagesPoll: startMessagesPoll, startDOMObserver: startDOMObserver, _lastUserSent: function(){ return _lastUserSent; } }
    };
