/* Draw Together - robust single-file implementation
   - Individual-only canvases (owner: 'me' by default)
   - Tools: brush, eraser, line, rect, circle, polygon (user-controlled sides)
   ...
   - Partner simulation:
       * Background agent (tick-based) may edit partner-owned canvases and optionally share
       * Optionally schedule partner draw+share after the app receives a partner message
   - Defensive initialization and detailed runtime error reporting in window.__drawTogetherError
*/
(function () {
  // Defensive wrapper
  try {
    ...
    // Partner background agent tuning (change at runtime via API below)
    let PARTNER_TICK_INTERVAL = 10 * 60 * 1000; // 10 minutes
    let PARTNER_DRAW_PROB = 0.021;              // per tick
    let PARTNER_SHARE_PROB = 1;              // per tick share if edited

    // Follow-up behavior when partner messages appear
    let PARTNER_DRAW_AFTER_SEND_PROB = 1;     // chance to draw after partner message
    let PARTNER_FORCE_SHARE_ON_FOLLOWUP = true; // if true, partner sends followup snapshot
    ...
    const notify = (t, type='info', time=3000) => { if (typeof showNotification === 'function') try { showNotification(t, type, time); } catch(e){ console.log(type+':', t); } else console.log(type+':', t); };
    ...
    // ===== Drawing primitives (structured actions) =====
    ...
    // ===== Chat push helper (tries addMessage, falls back to window.messages) =====
    function pushChatMessage(msg) {
      ...
      if (Array.isArray(window.messages)) {
        ...
        try { if (typeof renderMessages === 'function') renderMessages(); } catch (e) { /* ignore */ }
        throttledSave();
      } else {
        console.warn('[drawTogether] cannot push chat message (no addMessage / messages[])');
      }
    }
    ...
    // ===== Partner doodle generator & agent =====
    ...
    // ===== Schedule follow-up after partner message =====
      ...
      try {
        ...
        setTimeout(async () => {
          ...
          if (PARTNER_FORCE_SHARE_ON_FOLLOWUP) {
            ...
            pushChatMessage(out);
            canvas.lastSentHash = checksumSnapshotDataURL(dataURL);
            canvas.partnerEdited = false;
            await saveCanvasesLocal(canvasesCache);
            throttledSave();
          }
    ...
    // Hook app insertion points to detect partner messages
    if (typeof addMessage === 'function' && !addMessage.__drawTogetherWrapped) {
      const _orig = addMessage;
      addMessage = function(msg) {
        try { _orig.apply(this, arguments); } catch (e) { try { _orig(msg); } catch (e2) { console.warn('[drawTogether] addMessage wrapper error', e2); } }
    ...
    window.drawTogether = {
      ...
      setPartnerAfterSendProb: function(p) { PARTNER_DRAW_AFTER_SEND_PROB = Math.min(1, Math.max(0, Number(p)||PARTNER_DRAW_AFTER_SEND_PROB)); },
      setPartnerForceShareOnFollowup: function(b) { PARTNER_FORCE_SHARE_ON_FOLLOWUP = !!b; }
    };

    // init when DOM is ready (or immediately if already ready)
    async function init() {
      await loadAndCache();
      startPartnerAgent();
      console.debug('[drawTogether] ready');
      // optional: bind to button if present
    ...
    console.error('[drawTogether] initialization error', err);
  }
})();
