/* js/draw-together.js
   Durable Draw Together module with polygon tool and correct chat-send behavior.
   - Implements polygon preview and finalized polygons.
   - When sending to chat from user: creates a user-sent image message (image field + canvas meta).
   - When partner shares: creates partner-sent image message compatible with addMessage.
...
(function () {
  try {
    ...
    (function moduleBody() {
      ...
      const PARTNER_TICK_INTERVAL = 60 * 1000;
      const PARTNER_DRAW_PROB = 0.03;
      const PARTNER_SHARE_PROB = 0.25;
      ...
      function redrawFromActions(canvasEl, actions) { if (!canvasEl || !canvasEl.getContext) return; const ctx = canvasEl.getContext('2d'); ctx.clearRect(0,0,canvasEl.width, canvasEl.height); for (const a of (actions || [])) { try { drawAction(ctx, a); } catch (e) { console.warn('[drawTogether] drawAction failed for action', e, a); } } }
      ...
      function createCanvasObject(opts={}) {
        const now = Date.now(); const owner = opts.owner || 'me'; const shared = !!opts.shared; const id = uid('canvas');
        const o = { id, title: opts.title || 'Untitled', owner: shared ? 'shared' : owner, shared: shared, createdAt: now, lastModifiedAt: now, expiresAt: now + TWO_DAYS_MS, actions: [], lastSentHash: null, partnerEdited: false, meta: opts.meta || {} };
        canvasesCache.unshift(o); saveCanvasesLocal(canvasesCache).catch(()=>{}); throttledSave(); return o;
      }
      ...
      function buildToolbar(container) {
        ...
        `;
        container.querySelectorAll('.canvas-tool-btn').forEach(btn => { btn.addEventListener('click', () => { container.querySelectorAll('.canvas-tool-btn').forEach(b => b.classList.remove('active')); btn.classList.add('active'); container.dataset.selectedTool = btn.dataset.tool; }); });
        const defaultBtn = container.querySelector('button[data-tool="brush"]'); if (defaultBtn) { defaultBtn.classList.add('active'); container.dataset.selectedTool = 'brush'; }
      }
      ...
      function openCanvasModal(canvasId) {
        const modal = document.getElementById('canvas-modal'); const canvasEl = document.getElementById('drawing-canvas'); const toolbar = document.getElementById('canvas-toolbar');
        ...
        modal.style.zIndex = '999999'; const content = modal.querySelector('.modal-content'); if (content) { content.style.opacity = '1'; content.style.transform = 'none'; }
        if (typeof showModal === 'function') { try { showModal(modal, modal.querySelector('.modal-content') || null); } catch(e) { modal.style.display = 'flex'; } } else { modal.style.display = 'flex'; }
        try { document.body.style.overflow = 'hidden'; } catch(e){}

        if (!toolbar.dataset.built) { buildToolbar(toolbar); toolbar.dataset.built = '1'; const undoBtn = document.getElementById('canvas-undo'); const clearBtn = document.getElementById('canvas-clear'); if (undoBtn) undoBtn.addEventListener('click', () => window.doUndo && window.doUndo()); if (clearBtn) clearBtn.addEventListener('click', () => window.doClear && window.doClear()); }

        let canvasObj = canvasId ? findCanvasById(canvasId) : null; if (!canvasObj) canvasObj = createCanvasObject({ title: canvasId ? 'Loaded' : 'New', shared:false }); activeCanvas = canvasObj;

        const permSelect = document.getElementById('canvas-permission-select'); if (permSelect) { permSelect.value = canvasObj.shared ? 'shared' : 'individual'; permSelect.onchange = function() { const val = permSelect.value; canvasObj.shared = val === 'shared'; canvasObj.owner = canvasObj.shared ? 'shared' : canvasObj.owner === 'shared' ? 'me' : canvasObj.owner; canvasObj.lastModifiedAt = Date.now(); saveCanvasesLocal(canvasesCache); }; }
        const lockCheckbox = document.getElementById('canvas-private-lock'); if (lockCheckbox) { lockCheckbox.checked = Date.now() >= canvasObj.expiresAt; lockCheckbox.disabled = true; }

        const sendBtn = document.getElementById('canvas-send-to-chat'); if (sendBtn) sendBtn.onclick = function(){ sendCanvasToChat(canvasObj); };
        const closeBtn = document.getElementById('canvas-save-close'); if (closeBtn) closeBtn.onclick = function(){ closeCanvasModal(); };

        initDrawingCanvas(canvasEl, canvasObj); redrawFromActions(canvasEl, canvasObj.actions || []);
      }

      function closeCanvasModal() {
        const modal = document.getElementById('canvas-modal'); if (!modal) return;
        if (typeof hideModal === 'function') { try { hideModal(modal); } catch (e) { modal.style.display = 'none'; } } else { modal.style.display = 'none'; const content = modal.querySelector('.modal-content'); if (content) { content.style.opacity = '0'; content.style.transform = 'translateY(20px) scale(0.95)'; } }
        try { document.body.style.overflow = ''; } catch(e){}
        activeCanvas = null; saveCanvasesLocal(canvasesCache).catch(()=>{});
      }

      function initDrawingCanvas(canvasEl, canvasObj) {
        if (!canvasEl) return; canvasEl.width = CANVAS_DEFAULT_W; canvasEl.height = CANVAS_DEFAULT_H; const toolbar = document.getElementById('canvas-toolbar'); const colorInput = document.getElementById('canvas-color'); const sizeInput = document.getElementById('canvas-size');
        let drawing = false; let currentPoints = []; let startPoint = null; let lastMovePoint = null;

        function canEdit(c) { if (!c) return true; if (Date.now() >= c.expiresAt) return false; if (c.owner === 'me') return true; if (c.owner === 'partner') return false; if (c.owner === 'shared') return true; return true; }
        const ctx = canvasEl.getContext('2d');

        function getPos(e) { const rect = canvasEl.getBoundingClientRect(); if (e.touches && e.touches[0]) { return { x: (e.touches[0].clientX - rect.left) * (canvasEl.width/rect.width), y: (e.touches[0].clientY - rect.top) * (canvasEl.height/rect.height) }; } else { return { x: (e.clientX - rect.left) * (canvasEl.width/rect.width), y: (e.clientY - rect.top) * (canvasEl.height/rect.height) }; } }

        function startDraw(e) { if (!canEdit(canvasObj)) { notify('This canvas is locked (expired or permission denied)', 'warning'); return; } drawing = true; currentPoints = []; startPoint = getPos(e); lastMovePoint = startPoint; currentPoints.push(startPoint); try { e.preventDefault(); } catch(e){} }

        function moveDraw(e) {
          if (!drawing) return; const p = getPos(e); lastMovePoint = p; currentPoints.push(p);
          ...
          if (tool === 'brush' || tool === 'eraser') {
            ...
            const sides = Math.max(3, Math.min(12, Math.round((sizeInput ? parseInt(sizeInput.value,10) : 4) / 2) + 3));
            const rotation = 0;
            const pts = makeRegularPolygonPoints(startPoint.x, startPoint.y, r, sides, rotation);
        ...
        function endDraw(e) {
          if (!drawing) return; drawing = false; const tool = (toolbar && toolbar.dataset.selectedTool) || 'brush'; let action = null;
          if (tool === 'brush' || tool === 'eraser') { action = { type:'stroke', tool: tool, points: currentPoints.slice(), color: tool === 'eraser' ? '#000000' : (colorInput ? colorInput.value : '#000'), width: sizeInput ? parseInt(sizeInput.value,10) : 4 }; }
          else if (tool === 'poly') {
            const p = lastMovePoint || startPoint; const dx = p.x - startPoint.x, dy = p.y - startPoint.y; const r = Math.sqrt(dx*dx + dy*dy); const sides = Math.max(3, Math.min(12, Math.round((sizeInput ? parseInt(sizeInput.value,10) : 4) / 2) + 3)); const pts = makeRegularPolygonPoints(startPoint.x, startPoint.y, r, sides); action = { type:'shape', shapeType:'polygon', points: pts, color: colorInput ? colorInput.value : '#000', width: sizeInput ? parseInt(sizeInput.value,10) : 4, fill: null };
          } else {
          ...
          if (action) { canvasObj.actions.push(action); canvasObj.lastModifiedAt = Date.now(); canvasObj.partnerEdited = false; saveCanvasesLocal(canvasesCache); throttledSave(); }
          currentPoints = []; startPoint = null; lastMovePoint = null; redrawFromActions(canvasEl, canvasObj.actions || []);
          try { e.preventDefault(); } catch(e){}
        }

        canvasEl.onpointerdown = startDraw; canvasEl.onpointermove = moveDraw; window.addEventListener('pointerup', endDraw); canvasEl.ontouchstart = startDraw; canvasEl.ontouchmove = moveDraw; canvasEl.ontouchend = endDraw; canvasEl.onmousedown = startDraw; canvasEl.onmousemove = moveDraw; canvasEl.onmouseup = endDraw;
      ...
      function sendCanvasToChat(canvasObj) {
        if (!canvasObj) return;
        const tmp = document.createElement('canvas'); tmp.width = CANVAS_DEFAULT_W; tmp.height = CANVAS_DEFAULT_H; const ctx = tmp.getContext('2d'); ctx.fillStyle = '#ffffff'; ctx.fillRect(0,0,tmp.width,tmp.height); for (const a of canvasObj.actions) drawAction(ctx, a);
        ...
        // Build message compatible with addMessage / existing system: user-sent image message
        const msg = {
        ...
        pushChatMessage(msg);
        canvasObj.lastSentHash = hash; saveCanvasesLocal(canvasesCache); throttledSave(); notify('Canvas sent to chat', 'success');
      }

      window.renderCanvasMessageNode = function(msg) { const wrapper = document.createElement('div'); wrapper.className = 'canvas-msg'; const img = document.createElement('img'); img.src = (msg && (msg.image || (msg.content && msg.content.snapshot))) || ''; img.style.maxWidth = '320px'; img.style.borderRadius = '8px'; img.style.boxShadow = '0 6px 18px rgba(0,0,0,0.12)'; wrapper.appendChild(img); return wrapper; };

      function randomBetween(a,b) { return a + Math.random() * (b - a); } function randInt(a,b) { return Math.floor(randomBetween(a,b+1)); } function randomColor() { const h = Math.floor(Math.random() * 360); const s = randInt(40,95); const l = randInt(30,70); return `hsl(${h} ${s}% ${l}%)`; }
      ...
      function partnerAgentTick() {
        try {
          if (!canvasesCache.length) return; if (Math.random() > PARTNER_DRAW_PROB) return;
          const candidates = canvasesCache.filter(c => { if (Date.now() >= (c && c.expiresAt || 0)) return false; if (!c.shared && c.owner === 'me') return false; return true; }); if (!candidates.length) return;
          const canvas = candidates[randInt(0,candidates.length-1)]; const count = randInt(1,6);
          for (let i=0;i<count;i++) { const action = Math.random() > 0.45 ? generateRandomStroke() : generateRandomShape(); canvas.actions.push(action); }
          canvas.lastModifiedAt = Date.now(); canvas.partnerEdited = true; saveCanvasesLocal(canvasesCache).catch(()=>{}); throttledSave();

          if (Math.random() < PARTNER_SHARE_PROB) {
            const tmp = document.createElement('canvas'); tmp.width = CANVAS_DEFAULT_W; tmp.height = CANVAS_DEFAULT_H; const ctx = tmp.getContext('2d'); ctx.fillStyle = '#fff'; ctx.fillRect(0,0,tmp.width,tmp.height); for (const a of canvas.actions) drawAction(ctx, a); const dataURL = tmp.toDataURL('image/png'); const hash = checksumSnapshotDataURL(dataURL);
            if (canvas.lastSentHash !== hash) {
              if (canvas.partnerEdited) {
                ...
                pushChatMessage(msg);
                canvas.lastSentHash = hash; canvas.partnerEdited = false; saveCanvasesLocal(canvasesCache).catch(()=>{}); throttledSave(); try { window._sendPartnerNotification && window._sendPartnerNotification(partnerName, '发送了画布'); } catch(e){}
              }
            }
          }
      ...
      let partnerAgentHandle = null; function startPartnerAgent() { if (partnerAgentHandle) clearInterval(partnerAgentHandle); partnerAgentHandle = setInterval(partnerAgentTick, PARTNER_TICK_INTERVAL); }

      window.drawTogether = { openCanvasModalById: function(id) { try { openCanvasModal(id); } catch(e){ console.warn('[drawTogether] openCanvasModalById error', e); } }, newCanvas: function(opts){ try { const c = createCanvasObject(opts||{}); saveCanvasesLocal(canvasesCache); return c; } catch(e){ console.warn('[drawTogether] newCanvas error', e); return null; } }, listCanvases: function(){ return canvasesCache.slice(); }, load: async function() { await loadAndCache(); return canvasesCache; }, saveAll: function(){ return saveCanvasesLocal(canvasesCache); } };

      document.addEventListener('DOMContentLoaded', async function() { try { await loadAndCache(); startPartnerAgent(); console.debug('[drawTogether] ready'); const canvasBtn = document.getElementById('canvas-btn'); if (canvasBtn) { canvasBtn.addEventListener('click', () => { const c = createCanvasObject({ title: 'My Canvas', owner: 'me', shared: false }); openCanvasModal(c.id); }); } } catch (e) { console.warn('[drawTogether] DOMContentLoaded init failed', e); } });

    })();

    window.__drawTogetherLoaded = true; window.__drawTogetherError = null; console.debug('[drawTogether] loaded successfully');
  } catch (err) { try { window.__drawTogetherLoaded = false; window.__drawTogetherError = (err && err.stack) ? err.stack.toString() : String(err); } catch(e){} console.error('[drawTogether] top-level initialization error', err); }
})();
